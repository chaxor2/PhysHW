			...
			\begin{enumerate}[(a)]
				...
				\item Starting with versions of equations 10.1 and 10.2 from Simon's Solid State Basics adapted for different masses and same spring cosntants:
					...
					Therefore we have :
						...
						\begin{equation*}
							\begin{aligned}
								\Rightarrow m_1 \omega^2 A_x = \kappa (A_y  - A_x + A_y e^{iqa} - A_x ) \\
											m_2 \omega^2 A_y = \kappa (A_x e^{-iqa} - A_y + A_x - A_y ) 
							\end{aligned}
						\end{equation*}

						\begin{equation*}
							\begin{aligned}
								\Rightarrow m_1 \omega^2 A_x = \kappa (A_y (1+e^{iqa}) - 2A_x ) \\
											m_2 \omega^2 A_y = \kappa (A_x (1+e^{iqa}) - 2A_y ) 
							\end{aligned}
						\end{equation*}
					solving for $\omega$ gives:
						\begin{equation*}
							\omega^2 = \kappa(\frac{1}{m_1} + \frac{1}{m_2}) \pm \kappa \sqrt{\frac{1}{m_1^2} + \frac{1}{m_2^2} + \frac{2}{m_1 m_2} cos(qa)}
						\end{equation*}
				\item Optical ($+$ for $\pm$) and Acoustic mode ($-$ for $\pm$) frequencies at $q=0$ and $q=\frac{\pi}{a}$:
					\begin{itemize}
						\item Optical: \\

							$q=0:$ The atoms move towards and away from each other within each unit cell with frequency:
								\begin{equation*}
									\omega_+\big|_{q=0} = 2\kappa(\frac{1}{m_1} + \frac{1}{m_2})
								\end{equation*}
								\centerline{\includegraphics[scale=0.7]{mass_spring_qa0_optical.png}}

							$q=\frac{\pi}{a}:$ The atoms move towards and away from each other amongst each unit cell. 
								\begin{equation*}
									\omega_+ = \sqrt{\frac{2\kappa}{m_2}}
								\end{equation*}
								\centerline{\includegraphics[scale=0.7]{mass_spring_qapi_optical.png}}

						\item Acoustic: \\

							$q=0:$ The atoms move in phase with one another.
								\begin{equation*}
									\omega_- = 0
								\end{equation*}
								\centerline{\includegraphics[scale=0.7]{mass_spring_qa0_ac.png}}

							$q=\frac{\pi}{a}:$  The atoms wihin each unit cell move in phase but switch each unit cell.
								\begin{equation*}
									\omega_- = \sqrt{\frac{2\kappa}{m_1}}
								\end{equation*}
								\centerline{\includegraphics[scale=0.7]{mass_spring_qapi_ac.png}}
					\end{itemize}
					In the low energy regime (long wavelength, small $q$), the frequency is proportional to the phonon wave vector $q$ in a linear fashion. We notice this from the phonon dispersion in the low $q$ region for the acoustic branch. Equation 10.7 in Simon shows us that the sound velocity is:
								\begin{equation*}
									v_s\big|_{q=0} = \frac{d\omega_-}{dq}\big|_{q=0}
								\end{equation*}	
								\begin{equation*}
									\Rightarrow v_s\big|_{q=0} = \sqrt{\frac{\kappa}{2(m_1+m_2)}}a
								\end{equation*}	
					The group velocity is:
								\begin{equation*}
									v_g\big|_{q=\frac{\pi}{a}} = \frac{d\omega_\pm}{dq}\big|_{q=\frac{\pi}{a}}
								\end{equation*}	
								\begin{equation*}
									\Rightarrow v_g\big|_{q=\frac{\pi}{a}} = 0
								\end{equation*}

					\item The phonon dispersion is shown below: \\
						\centerline{\includegraphics[scale=0.7]{phonon_disp.png}}
						There are 2 branches in each unit cell and $2N$ number of modes for $N$ unit cells.
